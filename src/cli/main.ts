...
import type { Phase, PhaseOutcome } from "../dag/core.js";
...
import type { VulnerabilityClass } from "../fot/core.js";
import { emitManifest } from "../manifest/integration.js";
import { loadAllManifests } from "../manifest/writer.js";

async function cmdScan(repoPath: string, language: string): Promise<number> {
  ...
  if (overallSuccess) {
    await autoDeposit(
      scanId,
      language,
      "sql_injection" as VulnerabilityClass,
      `Scan ${scanId.slice(0, 8)} succeeded for ${language}`,
      ["--timeout 300000"],
    );
  }
...
async function cmdProposals(): Promise<number> {
