...
import { type InsightCapsule, type VulnerabilityClass, buildCapsule } from "./core.js";
 ...
 */
async function scanSucceeded(scanId: string): Promise<boolean> {
...
export async function autoDeposit(
  ...
  console.error(
    `[codex-security-evo] FoT: deposited insight ${capsule.id.slice(0, 16)}` +
