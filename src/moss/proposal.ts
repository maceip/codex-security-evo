...
export const MOSS_DIR: string = join(homedir(), ".codex-security", "moss");

/**
 * Generate a proposal id from a failure class + language.
 */
function proposalId(failureClass: string, language: string): string {
 ...
 */
export async function loadPendingProposals(): Promise<MossProposal[]> {
  ...
  const proposals: MossProposal[] = [];
  for (const file of files) {
    if (!file.endsWith(".json")) continue;
    try {
      const raw = await readFile(join(MOSS_DIR, file), "utf8");
      const p = JSON.parse(raw) as MossProposal;
      if (p.accepted === null) {
        proposals.push(p);
      }
    } catch {
...
export async function evaluateThreshold(
  ...
  const newProposals: MossProposal[] = [];

  for (const group of groups) {
    if (group.evidence.length < PROPOSAL_THRESHOLD) continue;

    const key = `${group.failure_class}:${group.language}`;
    if (existingKeys.has(key)) continue;

    const proposal: MossProposal = {
...
export async function recordDecision(
  ...
  try {
    ...
    const proposal = JSON.parse(raw) as MossProposal;
