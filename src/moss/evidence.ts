...
import { type MossEvidence, type EvidenceGroup } from "./core.js";
import type { DagEdge } from "../dag/core.js";

/**
 * Convert a DAG edge into MossEvidence, if it represents a failure.
 */
function edgeToEvidence(edge: DagEdge, language: string): MossEvidence | null {
  if (edge.outcome.succeeded) return null;
  return {
...
export function collectEvidence(
  ...
  const groups: Record<string, MossEvidence[]> = {};
 ...
 */
export function groupCount(group: EvidenceGroup): number {
