 ...
 */
export async function promptPendingProposals() {
    const proposals = await loadPendingProposals();
    if (proposals.length === 0)
        return 0;
    let decided = 0;
    for (const prop of proposals) {
        const accepted = await confirmOrSkip(`MOSS: ${prop.failure_class} / ${prop.language}: ${prop.suggestion} ` +
            `(${prop.trigger_count} failures, threshold ${PROPOSAL_THRESHOLD})`, true);
